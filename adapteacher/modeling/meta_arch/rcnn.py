...
class DAobjTwoStagePseudoLabGeneralizedRCNN(GeneralizedRCNN):
    ...
    def forward(
        self, batched_inputs, branch="supervised", given_proposals=None, val_mode=False, return_features=False, perturb=False
    ):
        """
        Args:
        ...
        if (not self.training) and (not val_mode):  # only conduct when testing mode
            output = self.inference(batched_inputs, return_features=return_features, perturb=perturb)
            return output
    ...
    def inference(
        ...
        if not self.original_params and perturb:
            ignoreNames = "D_img"
            ...
            n_params = sum([
                p.numel() for n, p in self.named_parameters() if not ignoreNames in n
            ])
            ...
            for n, p in self.named_parameters():
                ...
                p.data = self.original_params[n] + ip
                offset += size
        ...
        if detected_instances is None:
            ...
            results, predictions = self.roi_heads(images, features, proposals, None)
        else:
