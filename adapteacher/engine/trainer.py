...
#     verify_results,
# )
from adapteacher.evaluation import (
    verify_results,
    DatasetEvaluator,
    DatasetEvaluators,
    inference_on_dataset,
...
class ATeacherTrainer(DefaultTrainer):
    ...
    # modelSelect: rewrite test method.
    @classmethod
    def test(cls, cfg, model, evaluators=None, data_loader=None, return_features=False, perturb=False):
        """
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            if not data_loader:
                data_loader = cls.build_test_loader(cfg, dataset_name)  # cityscapes_foggy_val
            else:
                ...
            ...
            if return_features:
                results_i, preds_gallery, features_gallery = inference_on_dataset(model, data_loader, evaluator, return_features=return_features)
            else:
                results_i, preds_gallery = inference_on_dataset(model, data_loader, evaluator, perturb=perturb)
        ...
        if return_features:
            return results, preds_gallery, features_gallery
        else:
            return results, preds_gallery
